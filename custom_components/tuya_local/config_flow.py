...
QR_IMAGE_PATH = "/api/tuya_local/qr/{token}"
QR_URL_EXPIRY = timedelta(minutes=15)
QR_STORAGE_KEY = "qr_images"
QR_VIEW_KEY = "qr_view"


class TuyaLocalQrView(HomeAssistantView):
    """Serve the cloud login QR code as a fixed-color PNG.

    The frontend QR selector takes its colors from theme variables, which
    renders an unscannable image with themes that do not define the --rgb-*
    variables, so the image is generated server side instead. Access
    requires the signed URL generated by the scan step.
    """

    url = QR_IMAGE_PATH
    name = "api:tuya_local:qr"
    requires_auth = True

    async def get(self, request: web.Request, token: str) -> web.Response:
        """Return the QR code stored for the given flow."""
        images = request.app[KEY_HASS].data.get(DOMAIN, {}).get(QR_STORAGE_KEY, {})
...
class ConfigFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    def _qr_code_url(self) -> str:
        """Store a freshly rendered QR image and return a signed URL for it.

        Rendered server side with fixed black-on-white colors because the
        frontend QR selector takes its colors from the active theme, which
        makes the code unreadable with some custom themes.
        """
        domain_data = self.hass.data.setdefault(DOMAIN, {})
        if not domain_data.get(QR_VIEW_KEY):
            self.hass.http.register_view(TuyaLocalQrView)
            domain_data[QR_VIEW_KEY] = True

        buffer = io.BytesIO()
        segno.make_qr(
            f"tuyaSmart--qrLogin?token={self.__qr_code}",
            error="q",
        ).save(buffer, kind="png", scale=5, dark="#000000", light="#ffffff")
        images = domain_data.setdefault(QR_STORAGE_KEY, {})
        images[self.flow_id] = buffer.getvalue()
        # Bound the cache in case many flows are abandoned before cleanup;
        # evict oldest first so other active flows keep their images.
        while len(images) > 16:
            images.pop(next(iter(images)))
        return async_sign_path(
            self.hass,
            QR_IMAGE_PATH.format(token=self.flow_id),
            QR_URL_EXPIRY,
        )

    @callback
    def async_remove(self) -> None:
        """Clean up the stored QR image when the flow ends."""
        images = self.hass.data.get(DOMAIN, {}).get(QR_STORAGE_KEY)
        if images:
            images.pop(self.flow_id, None)
    ...
    async def async_step_scan(
        ...
        if user_input is None:
            return self.async_show_form(
                step_id="scan",
                data_schema=vol.Schema({}),
                description_placeholders={"qr_url": self._qr_code_url()},
            )
        self.init_cloud()
        if not await self.cloud.async_login():
            ...
            errors = {"base": "login_error"}
            placeholders = dict(self.cloud.last_error or {})
            if response:
                self.__qr_code = response

            placeholders["qr_url"] = self._qr_code_url()
            return self.async_show_form(
                step_id="scan",
                errors=errors,
                data_schema=vol.Schema({}),
...
class OptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if device:
                # Merge over existing options so keys managed elsewhere
                # (e.g. calibration offsets) are not wiped.
                return self.async_create_entry(
