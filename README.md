# Multiple Subdevice supported Tuya Local component
...
## Limitation of original [tuya-local](https://github.com/make-all/tuya-local)
The Tuya hub or gateway can have multiple subdevices hehind it, more than 10 devices in my own case. The original [tuya-local](https://github.com/make-all/tuya-local) has the limitation to support multiple subdevices.
...
## Single shared network connection to rescure
This fork version creates one single network connection to the hub device which is shared by all subdevices behind that.
